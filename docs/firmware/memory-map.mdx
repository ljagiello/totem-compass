...
<Note>
  The "retained across deep sleep" property of the RTC regions is a property of the
  ESP32, not a description of how this firmware behaves. Totem's application never calls
  `machine.deepsleep()`; it uses RTC memory to carry three small frames across a
  **reset** — see [boot flow](/firmware/boot-flow) and the frame format on the
...
| `0x3ff80000`–`0x3ff82000` | RTC fast (data bus) | retained data across reset and deep sleep |
...
| `0x50000000`–`0x50002000` | RTC slow | retained across reset and deep sleep |
