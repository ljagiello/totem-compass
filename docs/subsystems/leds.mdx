...
| `disconn_animation` | the charger going in, despite the name — it is the task name `Compass.power_conn_new` runs under, and that coroutine waits for `v_in` to read high three 100 ms samples running and then launches `powerup_animation` with the current volts |
