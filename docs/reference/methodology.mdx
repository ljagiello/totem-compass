...
2. **Distinctive-symbol coverage.** For each module, the identifiers in its `qstr_table`
   ...
   `f_lib/logger.py`, `f_lib/bitwise.py` and `f_lib/async_helpers.py` each score zero.
...
<Steps>
  ...
  <Step title="Diff">
    ...
    added, removed and changed functions plus the qstr and constant tables. 74 modules are
    identical, 20 changed and 2 were removed. See
