...
`power_conn_new` does once `v_in` has read high three samples running — a
contact that bounces on the way into the socket does not set it off. At
...
frames stopping. A pack on the charger never hits that cutoff, however
flat it reads. The mode is not settable by hand, because the next battery
