...
| `pos <lat> <lon> [accuracy m]`, `pos off` | report a GNSS fix, or none (the default) |
| `heading <deg>` | compass azimuth to report |
| `sos on\|off` | the SOS flag |
| `unbond <mac>` | delete a peer and send it the `(0,2)` notice |
| `log debug\|info` | log level; `debug` logs every frame heard and sent, as hex |
