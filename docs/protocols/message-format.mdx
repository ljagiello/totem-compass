...
picks `EXTENDED[key][len(frame)]` on an exact match, and for command 0 any frame of at least 72
bytes uses the 72 variant (`Non-extended payload being used` otherwise). `compass_mesh` only
...
`gen_peer_msg` fills `ENOW_PEER_BUFF = bytearray(108)` and always sends all **108 bytes**.
Command 0 is the status, 1 a bond request or confirmation, 2 an unbond notice.
...
| 25 | `h` | unscheduled | always 0; a receiver that sees non-zero does not predict the sender's next window |
...
| 43 | `e` | battery voltage | half float, MicroPython's encoder (rounds ties up) |
...
A name longer than 32 bytes does not leave room for the fields after it, and `gen_peer_msg`
fails. The buffer is reused, so bytes after the battery field can hold leftovers of a longer
earlier name.
...
| `Parser.recent` / `add_recent` | UIDs seen, each kept for its expiry + 15 s (clamped to 15-400 s), at most 512 |
| `MSG_EXP_MSECS` | default lifetime of a `recent` entry (150000 ms) |
| `MESH_PEER_MSG_LIMIT` | locate requests per stale peer (999) |
| `msg_offset_ms` | per-peer estimate of the sender's radio-window phase, from peer frame receive times |
